...
class LBFGS():
    ...
    def _gather_flat_grad(self):
        return self.scorer.total_gradient

    def _add_grad(self, step_size, update):
        self.x += step_size * update
    ...
    def step(self, closure):
        ...
        orig_loss = self.scorer.get_score(self.x)
        ...
        while n_iter < max_iter:
            ...
            if n_iter != max_iter:
                ...
                loss = float(self.scorer.get_score(self.x))
                flat_grad = self._gather_flat_grad()
                opt_cond = np.abs(flat_grad).max() <= tolerance_grad
                ls_func_evals = 1
            ...
            # lack of progress
            if np.abs(d * t).max() <= tolerance_change:
                break
